...
\begin{listing}[!ht]
    ...
    \end{pythoncode}
\end{listing}
In this listing, \mintinline{python}{sympy} was defined to be called \mintinline{python}{sp} and from \mintinline{python}{sympy} we directly imported some packages, too, which are needed later on.
...
Now, we are also using \mintinline{python}{sympy.abc} for displaying some standard symbols.
\begin{listing}[!ht]
...
\begin{figure}[!ht]
    ...
    \end{align}
\end{figure}
Looking at \cref{fig:intwp2barout}, this is exactly the integral, which we want to compute.
Using the command \mintinline{python}{.doit(conds='none')}, we can actually calculate the given integral, where we assume, that all given constants are real.
We are also using \mintinline{python}{.simplify()} here to make the output more readable as well as more comparable to the actual function we want to check.
\begin{listing}[!ht]
...
\begin{listing}[!ht]
    ...
    \end{pythoncode}
\end{listing}
We can print this equation, using \mintinline{python}{display} again.
\begin{listing}[!ht]
...
We can do this by just using \mintinline{python}{Eq(..)} from the package \mintinline{python}{SymPy}.
\mintinline{python}{factor(..)} just tries to factor the given variables to make the comparison easier.
\begin{listing}[!ht]
    ...
    \end{pythoncode}
\end{listing}
This code just displays \mintinline{python}{True}, which is exactly what we wanted to have.